...
function renderApplyPatchCall(
	...
	return new Text(
		summarizePatchCall(patchText, theme, context?.expanded === true),
		0,
		0,
	);
}
...
function applyLocalDecoration(
	...
	const overrideExisting = adapter.overrideExistingRenderers === true;

	if (adapter.renderCall && (overrideExisting || typeof decorated.renderCall !== "function")) {
		decorated.renderCall = adapter.renderCall;
	}
	if (adapter.renderResult && (overrideExisting || typeof decorated.renderResult !== "function")) {
		decorated.renderResult = adapter.renderResult;
	}
...
function decorateToolForDisplay(
	...
	if (!api) {
		...
		Object.assign(tool, applyLocalDecoration(tool, adapter));
		return;
	}
...
function adapterForToolName(toolName: string): ToolDisplayAdapter | undefined {
	...
	if (toolName === "apply_patch") {
		return {
			...
			renderResult: (result, options, theme) =>
				renderApplyPatchResult(result, options, theme),
		};
	}
